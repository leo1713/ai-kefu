...
async def test_embedding_stored(client: AsyncClient, mock_embed: None) -> None:
...
async def test_upload_creates_chunks(client: AsyncClient, mock_embed: None) -> None:
...
async def test_create_and_list_collection(client: AsyncClient) -> None:
    r = await client.post("/api/v1/knowledge/collections", json={"name": "测试集合", "description": "单元测试"})
    assert r.status_code == 201
    coll_id = r.json()["id"]

    listing = await client.get("/api/v1/knowledge/collections")
...
async def test_delete_collection(client: AsyncClient) -> None:
    r = await client.post("/api/v1/knowledge/collections", json={"name": "待删除集合"})
    assert r.status_code == 201
    coll_id = r.json()["id"]

    del_r = await client.delete(f"/api/v1/knowledge/collections/{coll_id}")
...
async def test_list_and_delete_document(client: AsyncClient, mock_embed: None) -> None:
    ...
    del_r = await client.delete(f"/api/v1/knowledge/documents/{doc_id}")
    assert del_r.status_code == 204

    docs_after = await client.get("/api/v1/knowledge/documents")
